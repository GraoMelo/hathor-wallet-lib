...
const transaction = {
  ...
  /**
   * Generate a transaction and returns it, without propagating to the network.
   **/
  ...
  sendTransaction(data, pin, options) {
    try {
      transaction.prepareData(data, pin, options);
    } catch (e) {
   ...
   * already been prepare, i.e., it must be complete.
   *
   * @param {Object} data Object with inputs and outputs
   * {
   *  'inputs': [{'tx_id', 'index', 'token', 'address'}],
